    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">Gestiona las marcas de productos</p>
                </div>
            </div>
        </template>
        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold">{{ safeBrands.length }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold">{{ activeBrands }}</p>
                            </div>
                            <div class="p-3 bg-white bg-opacity-20 rounded-lg">
                ...
                <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                    <div class="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-b border-gray-200">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-4">
                                <span class="text-sm text-gray-500">{{ safeBrands.length }} marcas registradas</span>
                                <button
                    ...
                    <div class="p-6">
                        <div v-if="safeBrands.length === 0" class="text-center py-12">
                            ...
                            <p class="text-sm text-gray-400 mt-1">Comienza agregando tu primera marca</p>
                            <button
                                @click="openCreateModal"
                                    ...
                                    <tbody class="divide-y divide-gray-100">
                                        <tr v-for="brand in filteredBrands" :key="brand.id" class="hover:bg-gray-50 transition-colors">
                                            <td class="py-4 px-4 text-sm font-medium text-gray-900">
                                                {{ brand.formatted_id || 'M' + String(brand.id).padStart(4, '0') }}
                                            </td>
                                            <td class="py-4 px-4">
                                            ...
                                            <td class="py-4 px-4 text-center">
                                                ...
                                                >
                                                    <div class="h-1.5 w-1.5 rounded-full mr-1.5" :class="[
                                                        brand.is_active ? 'bg-green-600' : 'bg-red-600'
                                                    ]" />
        ...
        >
            <div class="bg-white">
                ...
                <!-- Contenido -->
                <div class="px-6 py-6" v-if="selectedBrand">
                    <div class="space-y-4">
                        <div>
                            <label class="text-sm font-medium text-gray-500">Nombre:</label>
                            <p class="text-sm text-gray-900 mt-1">{{ selectedBrand.name }}</p>
                        </div>
                        <div>
                            <label class="text-sm font-medium text-gray-500">Descripción:</label>
                            <p class="text-sm text-gray-900 mt-1">{{ selectedBrand.description || 'Sin descripción' }}</p>
                        </div>
                        <div>
                            ...
                            <div class="mt-1">
                                ...
                                >
                                    {{ selectedBrand.is_active ? 'Activo' : 'Inactivo' }}
                                </span>
                            </div>
                        </div>
                    </div>
...
const activeBrands = computed(() => {
    return safeBrands.value.filter(brand => brand.is_active).length
})

const totalProducts = computed(() => {
    return safeBrands.value.reduce((total, brand) => {
        const count = parseInt(brand.products_count) || 0
        return total + count
    }, 0)
})
...
const averageProducts = computed(() => {
    ...
    const count = safeBrands.value.length
    return count > 0 ? Math.round(total / count) : 0
})

const filteredBrands = computed(() => {
    if (!searchQuery.value) {
        return safeBrands.value
    }
    
    const query = searchQuery.value.toLowerCase()
    return safeBrands.value.filter(brand => 
...
const handleSearch = () => {
    // La búsqueda se maneja reactivamente a través de filteredBrands
}
...
const submitForm = () => {
    if (isEditing.value) {
        form.put(route('admin.brands.update', selectedBrand.value.id), {
            onSuccess: () => {
                closeModal()
                success('Marca actualizada', 'La marca se ha actualizado correctamente')
            },
            onError: () => {
                error('Error al actualizar', 'No se pudo actualizar la marca')
            }
        })
    } else {
        form.post(route('admin.brands.store'), {
            onSuccess: () => {
                closeModal()
                success('Marca creada', 'La marca se ha creado correctamente')
            },
            onError: () => {
                error('Error al crear', 'No se pudo crear la marca')
            }
        })
    }
}
...
const deleteBrand = () => {
    deleteForm.delete(route('admin.brands.destroy', selectedBrand.value.id), {
        onSuccess: () => {
            closeDeleteModal()
            success('Marca eliminada', 'La marca se ha eliminado correctamente')
        },
        onError: () => {
            error('Error al eliminar', 'No se pudo eliminar la marca')
        }
    })
}
